...
uint_32 Vcycle_AIMGM(uint_32* M_array, uint_32 M_agrray_len,
                     double** offlaplacex, double** offlaplacey, complex** mainlaplace,
                     complex deltaFTFC, complex* rhs, complex* sol, double tol);
uint_32 MGMManager(uint_32* M_array, uint_32 M_agrray_len,
               double** offlaplacex, double** offlaplacey, complex** mainlaplace,
               complex* rhs, complex* sol, double tol);
void MGM(uint_32* M_array, uint_32 M_array_len,
	 double** offlaplacex, double** offlaplacey, complex** mainlaplace,
...
void get_residual(double* offlaplacex, double* offlaplacey,
                  ...
                  complex* residual);
void In_Cplt_LU_Sol(double* offlaplacex, double* offlaplacey,
...
void tracking(double* offlaplacex, complex* mainlaplace, complex* x, complex* b,
              uint_32 size);
void AXLGS_smoother(double* offlaplacex, double* offlaplacey,
                    complex* mainlaplace, complex* initialgs, complex* rhs, uint_32 M, uint_32 time,
                    bool iszeroinitial);
...
void Time_frac_diffusion_2Deq_AIMGM_solver(uint_32 N, uint_32 level, double X_L,
	...
	if (tol<=0) {
		std::cerr<<"FAI warning: the tolerance must be positive number!!!"<<std::endl;
	}
	std::cerr << "FAIsolver Starting" << std::endl;
	...
	mainlaplace=mainlaplace_generate(M_array, M_array_len, offlaplacex, offlaplacey,
	                                 X_L, X_R, Y_Low, Y_Upp);
...
void Time_frac_diffusion_2Deq_BFSMGM_solver(uint_32 N, uint_32 level, double X_L,
	...
	for (i=1; i<N; i++) {
		...
		// We now need to 'advance' the rhs
		for(j=0; j<i;j++) {
			for(uint_32 k=0;k<Msquare; ++k) {
				rhs[i][k] = cpminus(rhs[i][k],cpmultr(rhs[j][k],Frac_div_appro_coeff[i-j]));
			}
		}
...
uint_32 Vcycle_BLTDTDB(uint_32* M_array, uint_32 M_agrray_len,
	...
	while(true) {
		...
		AXLGS_smoother(offlaplacex[0], offlaplacey[0], mainlaplace[0], sol, rhs,
		               M_array[0], timepre, iszeroinital);
		residual=new complex[M_array[0]*M_array[0]];
		get_residual(offlaplacex[0], offlaplacey[0], mainlaplace[0], rhs, sol,
		             M_array[0], residual);
...
void MGM(uint_32* M_array, uint_32 M_array_len,
           double** offlaplacex, double** offlaplacey, complex** mainlaplace,
           complex** rhsides, complex** solutions, bool isinitial) {
	//std::cerr << "MGM: M_array_len: " << M_array_len << std::endl;
	//std::cerr << "MGM: M_array: ";
	uint_32 Ms = M_array[0]*M_array[0];
	//report_vector(M_array, M_array_len);
	//std::cerr << std::endl;
	//std::cerr << "MGM rhsides[0] pointer:  " << rhsides[0] << std::endl;
	//std::cerr << "MGM: rhsides[0]: ";
	//report_vector(rhsides[0], 6);
	//std::cerr << std::endl;
	//std::cerr << "MGM: solutions[0]: ";
	//report_vector(solutions[0], 6);
	//std::cerr << std::endl;

	if (M_array[0] == 3) {
		In_Cplt_LU_Sol(offlaplacex[0], offlaplacey[0], mainlaplace[0], rhsides[0], solutions[0]);
		//std::cerr << "MGM: direct solve" << std::endl;
		return;
	}
	...
	//std::cerr << "MGM: ---Pre-Smoothing---" << std::endl;
	//std::cerr << "MGM: solutions[0]: ";
	//report_vector(solutions[0], 6);
	//std::cerr << std::endl;
	...
	//std::cerr << "MGM: ---Restrict---" << std::endl;
	//std::cerr << "MGM: residual: ";
	//report_vector(residual, 6);
	//std::cerr << std::endl;
	//std::cerr << "MGM: rhsides[1]: ";
	...
	//std::cerr << "MGM: solutions[1]: ";
	//report_vector(solutions[1], 6);
	//std::cerr << std::endl;

	//Recursive call on next layer down
	//std::cerr << "MGM: ---Start Recursive Call---" << std::endl;
	MGM(M_array+1,M_array_len-1, offlaplacex+1, offlaplacey+1,mainlaplace+1,rhsides+1,solutions+1, true); 
	//std::cerr << "MGM: ---End Recursive Call---" << std::endl;
	...
	//interpolation
	//std::cerr << "MGM: ---Interpolate---" << std::endl;
	...
	//post smoothing
	//std::cerr << "MGM: ---Post Smoothing---" << std::endl;
...
uint_32 Vcycle_AIMGM(uint_32* M_array, uint_32 M_array_len,
	...
	bool isinitial = true;
	do {
		MGM(M_array, M_array_len, offlaplacex, offlaplacey, mainlaplace, rhsides, solutions, isinitial);
		if(isinitial) {
			isinitial = false;
		}
...
uint_32 MGMManager(uint_32* M_array, uint_32 M_array_len,
                   double** offlaplacex, double** offlaplacey, complex** mainlaplace,
                   complex* rhs, complex* sol, double tol) {
	//std::cerr << "MGMManager: M_array_len: " << M_array_len << std::endl;
	//std::cerr << "MGMManager: M_array: ";
	//report_vector(M_array, M_array_len);
	//std::cerr << std::endl;
	uint_32 tempMs, Ms;
...
double** offlaplacex_generate(uint_32* M_array, uint_32 M_array_len, double X_L,
	...
	for (m=0; m<M_array_len; m++) {
		//std::cerr << "M_array[" << m << "]: " << M_array[m] << std::endl;
		//parameter settings
		Mp1=M_array[m]+1;
...
double** offlaplacey_generate(uint_32* M_array, uint_32 M_array_len, double X_L,
                              double X_R, double Y_Low, double Y_Upp) {
	//std::cerr << "offlapacey_generate called len: " << M_array_len << std::endl;
...
complex** mainlaplace_generate(uint_32* M_array, uint_32 M_array_len,
                               double** offlaplacex, double** offlaplacey, double X_L, double X_R,
                               double Y_Low, double Y_Upp) {
	//std::cerr << "mainlaplace_generate called len: " << M_array_len << std::endl;
	...
	for (m=0; m<M_array_len; m++) {
		...
		for (i=1; i<Mm1; i++) {
			mainlaplace[m][i].r=offlaplacex[m][i-1]+offlaplacex[m][i]+offlaplacey[m][i]
...
void get_residual(double* offlaplacex, double* offlaplacey,
                  ...
                  complex* residual) {
	//std::cerr << "get_residual called" << std::endl;
	...
	for (i=1; i<Mm1; i++) {
		residual[i]=cpplus(rhs[i], cpminus(cpplus(cpplus(cpmultr(sol[i-1],
		                                   offlaplacex[i-1]), \
		                                   cpmultr(sol[i+1], offlaplacex[i])), cpmultr(sol[indnexts+i], offlaplacey[i])), \
		                                   cpmult(mainlaplace[i], sol[i])));
	}

	residual[Mm1]=cpplus(rhs[Mm1], cpminus(cpplus(cpmultr(sol[Mm2],
	                                       offlaplacex[Mm2]), \
	                                       cpmultr(sol[indnexts+Mm1], offlaplacey[Mm1])), cpmult(sol[Mm1],
	                                               mainlaplace[Mm1])));
	...
	for (j=1; j<Mm1; j++) {
		...
		residual[indy]=cpplus(rhs[indy], cpminus(cpplus(cpplus(cpmultr(sol[indypre], \
		                      ...
		                      cpmultr(sol[indy+1], offlaplacex[indx])), cpmult(sol[indy],
		                              mainlaplace[indy])));
		...
		residual[indy+Mm1]=cpplus(rhs[indy+Mm1], cpminus(cpplus(cpmultr(sol[indy+Mm2], \
		                          offlaplacex[indx+Mm2]), cpplus(cpmultr(sol[indypre+Mm1],
		                                  offlaplacey[indypre+Mm1]), \
	...
	indy=Mm1*M;
	indx=Mm1*Mm1;
	residual[indy]=cpplus(rhs[indy], cpminus(cpplus(cpmultr(sol[indy+1],
	                      offlaplacex[indx]), \
	                      cpmultr(sol[indypre], offlaplacey[indypre])), cpmult(sol[indy],
	                              mainlaplace[indy])));

	for (i=1; i<Mm1; i++) {
		residual[indy+i]=cpplus(rhs[indy+i], cpminus(cpplus(cpmultr(sol[indypre+i],
		                        offlaplacey[indypre+i]), \
		                        cpplus(cpmultr(sol[indy+i-1], offlaplacex[indx+i-1]), cpmultr(sol[indy+i+1], \
		                                offlaplacex[indx+i]))), cpmult(sol[indy+i], mainlaplace[indy+i])));
	}

	residual[indy+Mm1]=cpplus(rhs[indy+Mm1], cpminus(cpplus(cpmultr(sol[indy+Mm2], \
	                          offlaplacex[indx+Mm2]), cpmultr(sol[indypre+Mm1], offlaplacey[indypre+Mm1])),
	                          cpmult(sol[indy+Mm1], \
	                                 mainlaplace[indy+Mm1])));
}
inline void In_Cplt_LU_Sol(double* offlaplacex, double* offlaplacey,
                           complex* mainlaplace, complex* rhs, complex* sol) {
	//std::cerr << "In_Cplt_LU_Sol called" << std::endl;
	...
	complex EofA[4], t;
	EofA[0]=mainlaplace[0];
	EofA[1].r=-offlaplacex[0];
	EofA[1].i=0;
	EofA[2].r=0;
	EofA[2].i=0;
	EofA[3].r=-offlaplacey[0];
	EofA[3].i=0;
...
inline void Restr_Operator(complex* rh, complex* f2h, uint_32 M) {
	...
	//std::cerr << "Restr_Operator M: " << M << std::endl;
	for (j=0; j<nM; j++) {
		indh=2*j*M;
		indhp1=indh+M;
		indhp2=indh+2*M;
		...
		for (i=0; i<nM; i++) {
			...
			tip1=ti+1;
			tip2=ti+2;
			f2h[ind2h+i]=cpplus(cpmultr(rh[indh+ti], 0.0625), cpplus(cpmultr(rh[indh+tip1],
			                    0.125), cpplus(cpmultr(rh[indh+tip2], 0.0625), cpplus(cpmultr(rh[indhp1+ti],
			                                   0.125), \
			                                   cpplus(cpmultr(rh[indhp1+tip1], 0.25), cpplus(cpmultr(rh[indhp1+tip2], 0.125),
			                                           cpplus(cpmultr(rh[indhp2+ti], 0.0625), cpplus(cpmultr(rh[indhp2+tip1], 0.125),
			                                                   cpmultr(rh[indhp2+tip2], 0.0625)))))))));
		}
	}
}
...
inline void Interp_Operator(complex* u2h, complex* uh, uint_32 M) {
	//std::cerr << "Interp_Operator M: " << M << std::endl;
	...
	for(j=0; j<Mm1; j++) {
		...
		for(i=0; i<Mm1; i++) {
			...
			uh[2*i+2+oddjmnm]=cpplus(uh[2*i+2+oddjmnm], cpmultr(cpplus(u2h[i+jm],
			                         u2h[i+1+jm]), 0.5));
			//compute the u_2i+1_2j+2 term in finer grid
			uh[2*i+1+evenjmnm]=cpplus(uh[2*i+1+evenjmnm], cpmultr(cpplus(u2h[i+jm],
			                          u2h[i+jp1m]), 0.5));
			//compute the u_2i+2_2j+2 term in finer grid
			uh[2*i+2+evenjmnm]=cpplus(uh[2*i+2+evenjmnm], cpplus(cpmultr(cpplus(u2h[i+jm],
			                          u2h[i+1+jm]), 0.25), cpplus(cpmultr(u2h[i+jp1m], 0.25), cpmultr(u2h[i+1+jp1m],
			                                  0.25))));
		}
		...
		uh[nMm2+evenjmnm]=cpplus(uh[nMm2+evenjmnm], cpmultr(cpplus(u2h[Mm1+jm],
		                         u2h[Mm1+jp1m]), 0.5));
		...
		uh[nMm1+evenjmnm]=cpplus(uh[nMm1+evenjmnm], cpmultr(cpplus(u2h[Mm1+jm],
		                         u2h[Mm1+jp1m]), 0.25));
	}
	...
	for(i=0; i<Mm1; i++) {
		...
		uh[2*i+2+evenjmnm]=cpplus(uh[2*i+2+evenjmnm], cpmultr(cpplus(u2h[i+jm],
		                          u2h[i+1+jm]), 0.25));
	}
...
inline void tracking(double* offlaplacex, complex* mainlaplace, complex* x,
                     complex* b, uint_32 size) {
	uint_32 i;
	...
	bmab[size-2]=cpplus(mainlaplace[size-1], cpmultr(beta[size-2],
	                    offlaplacex[size-2]));
...
inline void AXLGS_smoother(double* offlaplacex, double* offlaplacey,
                           complex* mainlaplace, complex* initialgs, complex* rhs, uint_32 M, uint_32 time,
                           bool iszeroinitial) {
	if (time<1) {
		std::cerr<<"FAI warning: the iteration time must be larger that zero!!"<<std::endl;
		return;
	}
	//std::cerr << "AXLGS_smoother called! M: " << M << std::endl;
	...
	if (iszeroinitial) {
		...
		for (j=1; j<Mm1; j+=2) {
			...
			offxcurinds=j*Mm1;
			//update the red line
			tracking(offlaplacex+offxcurinds, mainlaplace+veccurinds, initialgs+veccurinds,
			         rhs+veccurinds, M);
		}
		...
		for (i=0; i<M; i++) {
			temprhs[i]=cpplus(rhs[veccurinds+i], cpmultr(initialgs[vecpreinds+i],
			                  offlaplacey[vecpreinds+i]));
		}
